...
describe('Testing <BirthDatText /> [PT-PT]', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('it should return render correctly with a valid date and the current language is "ptPT"', () => {
    const { getByText } = render(renderBirthDayText('1994-02-21'));

    expect(getByText(`21 de ${MOCK_T_VALUE.toLowerCase()} de 1994`)).not.toBeNull();
  });

  it('it should render just an "-" when the date is invalid and the current language is "ptPT"', () => {

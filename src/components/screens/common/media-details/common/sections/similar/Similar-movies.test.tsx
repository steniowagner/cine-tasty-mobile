...
import {MovieDetail_movie_similar} from '@schema-types';
...
export const similarMovies = (length: number) =>
  Array(length)
    .fill({})
    .map((_, index) => ({
...
describe('<Similar /> - [Movies]', () => {
  ...
  describe('Renders correctly', () => {
    ...
    it('should render correclty when it has some data to show', async () => {
      ...
      const component = render(renderSimilar(similarMovies(length)));
  ...
  describe('Pressing the items', () => {
    ...
    it('should call "onPressItem" correctly when the user press smoe "similar-item"', async () => {
      ...
      const similar = similarMovies(length);

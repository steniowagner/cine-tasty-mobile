...
const getRandomZeroBasedIndex = <T>(array: T[]) => (Math.random() * (array.length - 1 - 0 + 1)) << 0

export const randomPositiveNumber = (max: number, min: number = 0) => Math.floor(Math.random() * (max - min + 1)) + min;
...
export const randomArrayElement = <T>(array: T[], avoid?: number[]) => {
    ...
    while (true) {
        let randomZeroBasedIndex = getRandomZeroBasedIndex(array);
